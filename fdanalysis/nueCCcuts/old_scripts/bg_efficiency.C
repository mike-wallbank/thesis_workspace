void bg_efficiency(TString inputname, TString outputname="hists_bgefficiencies.root",TString mvacut="0.8",TString friendfile=""){
  ...
  TString BACKGROUND_DEF="(!(abs(beamPdg)==14 && abs(neu)==12 && cc==1) && "+ INFV + ")";
  ...
  h_enu_sig->Sumw2();
  h_enu_sig->GetXaxis()->SetTitle("E_{#nu_{#mu}} (GeV)");
  h_enu_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");
  ...
  h_enu_sel_sig->GetXaxis()->SetTitle("E_{#nu_{#mu}} (GeV)");
  h_enu_sel_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");

  MVASelection->Draw("Ev >> h_enu_sig",WEIGHTING+"*(("+BACKGROUND_DEF+"))");
  MVASelection->Draw("Ev >> h_enu_sel_sig",WEIGHTING+"*(("+BACKGROUND_DEF+")&&"+SELECTION+")");

  TEfficiency *h_eff_enu = new TEfficiency(*h_enu_sel_sig,*h_enu_sig);
  h_eff_enu->SetTitle(TString("eff;")+h_enu_sig->GetXaxis()->GetTitle()+TString(";Background efficiency (No units)"));
  //h_eff_enu->Draw();
  ...
  h_q2_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");
  ...
  h_q2_sel_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");

  MVASelection->Draw("Q2 >> h_q2_sig",WEIGHTING+"*("+BACKGROUND_DEF+")");
  MVASelection->Draw("Q2 >> h_q2_sel_sig",WEIGHTING+"*("+BACKGROUND_DEF+"&&"+SELECTION+")");

  TEfficiency *h_eff_q2 = new TEfficiency(*h_q2_sel_sig,*h_q2_sig);
  h_eff_q2->SetTitle(TString("eff;")+h_q2_sig->GetXaxis()->GetTitle()+TString(";#nu_{#mu}#rightarrow#mu efficiency (No units)"));
  ...
  h_lepangle_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");
  ...
  h_lepangle_sel_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");

  MVASelection->Draw("LepNuAngle*180/3.14159265359 >> h_lepangle_sig",WEIGHTING+"*("+BACKGROUND_DEF+")");
  MVASelection->Draw("LepNuAngle*180/3.14159265359 >> h_lepangle_sel_sig",WEIGHTING+"*("+BACKGROUND_DEF+"&&"+SELECTION+")");

  TEfficiency *h_eff_lepangle = new TEfficiency(*h_lepangle_sel_sig,*h_lepangle_sig);
  h_eff_lepangle->SetTitle(TString("eff;")+h_lepangle_sig->GetXaxis()->GetTitle()+TString(";#nu_{#mu}#rightarrow#mu efficiency (No units)"));
  ...
  h_lepmom_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");
  ...
  h_lepmom_sel_sig->GetYaxis()->SetTitle("No. #nu_{#mu}");

  MVASelection->Draw("sqrt(LepMomX*LepMomX + LepMomY*LepMomY + LepMomZ*LepMomZ) >> h_lepmom_sig",WEIGHTING+"*("+BACKGROUND_DEF+")");
  MVASelection->Draw("sqrt(LepMomX*LepMomX + LepMomY*LepMomY + LepMomZ*LepMomZ) >> h_lepmom_sel_sig",WEIGHTING+"*("+BACKGROUND_DEF+"&&"+SELECTION+")");

  TEfficiency *h_eff_lepmom = new TEfficiency(*h_lepmom_sel_sig,*h_lepmom_sig);
  h_eff_lepmom->SetTitle(TString("eff;")+h_lepmom_sig->GetXaxis()->GetTitle()+TString(";#nu_{#mu}#rightarrow#mu efficiency (No units)"));
