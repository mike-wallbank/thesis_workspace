void electronMVATuneDeltaCP(TString inputname, double deltaCP_A = 0, double deltaCP_B = TMath::Pi()/2.) {
  ...
  TString output_name = Form("output_deltaCP_%f_to_%f_electronMVATuneProj.root",deltaCP_A,deltaCP_B);
  ...
  histA->GetXaxis()->SetTitle("Electron-like PID score");
  ...
  tree->Draw("RecoENu:SelMVAElectron >> histA","("+oscWeightA+"*ProjectedPOTWeight)*1","goff");
  tree->Draw("RecoENu:SelMVAElectron >> histB","("+oscWeightB+"*ProjectedPOTWeight)*1","goff");
  ...
  for (int i_bin = 1; i_bin <= histA->GetXaxis()->GetNbins(); i_bin++){
    ...
    TString histA_proj_name = Form("h_deltaCP_%f_electronMVAGR%f",deltaCP_A,current_mva);
    ...
    TString h_chi2_name = Form("h_chi2_electronMVAGR%f",current_mva);
  ...
  g_chi2->Draw();
  g_chi2->GetXaxis()->SetTitle("Electron-like PID score (cut value)");
